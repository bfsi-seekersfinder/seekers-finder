...
const ProfileCard = ({Candidate}) => {
    ...
    const handleCopy = (field) => {
        if(!VeiwNumberAndEmail){
            ...
            if(field === 'number'){
            navigator.clipboard.writeText(condidate.mobileNo? condidate.mobileNo : condidate["mobile number"])
        ...
        <div  className=' border bg-white border-white rounded-md shadow-md'>
            ...
            <div className='flex flex-col w-[400px]   max-lg:w-[300px]  gap-4'>
                <div className='flex gap-4'>
                    ...
                    <span className='font-bold text-slate-600 tracking-wider capitalize'>{condidate.fullName? condidate.fullName:""} </span>
                    <span className='text-[14px] font-bold tracking-wider text-slate-500'>
                    {condidate.workExperience && condidate.workExperience[0] && condidate.workExperience[0]?.description 
                        ? condidate.workExperience[0].description 
                        : ""}
                    </span>
                    <span className='text-[12px]  tracking-wider text-slate-500'> {condidate.product? condidate.product.length>40 && !isViewMore? <> {condidate.product.slice(0, 40)}<span onClick={()=>handleViewMore()} className='text-cyan-500 cursor-pointer'>...view more</span> </>: condidate.product.length>40? <>{condidate.product} <span onClick={()=>handleViewMore()} className='text-cyan-500 cursor-pointer'>...view less</span>  </> : condidate.product: "Not available"} </span>
                    </div>
                    ...
                    <span><i className="ri-wallet-3-fill font-bold"></i> {condidate.currentCtc? condidate?.currentCtc? condidate.currentCtc : condidate.currentCtc.$numberDecimal.toString() : '0'} {condidate.currentCtc> 9999999 ? "(cr)" : "(lac)"} </span>
                    <span><i className="ri-map-pin-2-line font-bold"></i>{condidate.userLocation? condidate.userLocation.city:''}</span>
                    </div>
                    ...
                    >
                    {condidate.mobileNo 
                        ? `91+ ${VeiwNumberAndEmail ? condidate.mobileNo : String(condidate.mobileNo).slice(0, 4) + "XXXXXX"}`
                        : condidate["mobile number"]
                        ? `91+ ${VeiwNumberAndEmail ? String(condidate["mobile number"]) : String(condidate["mobile number"]).slice(0, 4) + "XXXXXX"}`
                        : "Contact Unavailable"}
                    </button>
                    ...
                    <button  onClick={()=>handleSeeSingleCandidateProfile(condidate._id)} className='border max-lg:text-[12px] border-slate-400 px-4 max-lg:px-2 py-0.5 hover:bg-slate-700 hover:text-white active:bg-slate-600 active:text-white  rounded-sm text-sm text-slate-700 cursor-pointer'>Profile</button>
                    <button  onClick={()=>handleViewcontacts(condidate._id)} className='border max-lg:text-[12px] border-slate-400 px-4 max-lg:px-2 py-0.5 hover:bg-slate-700 hover:text-white active:bg-slate-600 active:text-white  rounded-sm text-sm text-slate-700 cursor-pointer'>View Contacts</button>
            ...
            <div className='flex flex-col gap-2 text-sm w-full h-full text'>
                ...
                <div className=' py-4 '>
                    ...
                    <textarea 
                    placeholder='Suggest candidate updates..'
                    value={suggestionText}
                    onChange={(e) =>setsuggestionText(e.target.value)} 
                    className='border border-slate-500 w-full max-h-[50px] min-h-8 px-2 py-0.5 focus:outline-none rounded'>
                    </textarea>
                    <button onClick={()=>sendSuggestion(condidate._id)} className="bg-slate-500 text-white px-6 py-0.5 rounded cursor-pointer flex gap-2 items-center"
                    >
                ...
                <div className='flex flex-col items-center relative  gap-4 w-10  text-gray-600'>
                   <span className='cursor-pointer text-xl'><i className="ri-file-download-line"></i></span>    
                    <button onClick={()=>{handleSaveProfile(condidate._id)}} 
                    className='cursor-pointer text-xl'>{savedProfiles.length>0 && savedProfiles.some(item => item._id === condidate._id)? (<i className="ri-bookmark-fill"></i>) : (<i className="ri-bookmark-line"></i>)} 
                    </button>
                    <button onClick={()=>{
                        handleSuggetionBox()
                    }} className='cursor-pointer text-xl'><i className="ri-pen-nib-fill"></i></button>
                    <button onClick={()=>setisWhatsAppOpen(true)} className='cursor-pointer text-xl'><i className="ri-whatsapp-line"></i></button>
            ...
            <div className=' flex justify-between pr-2'>
                ...
                <span className={`${getOpenProfile?.length>0 && getOpenProfile?.includes(condidate._id)? "text-blue-600":"text-gray-400"}`}>
            ...
            <span className='cursor-pointer flex items-center justify-end w-full mt-2 h-[10px] text-slate-600'> <span className='text-[10px] font-light mr-1'>last update </span> <span className='text-[9px] font-light'> {condidate.updatedAt? getDuration(condidate.updatedAt) : "2 years"} </span></span>    
