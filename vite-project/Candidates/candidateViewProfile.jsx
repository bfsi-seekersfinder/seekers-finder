  ...
  return (
    <div className=" w-[65%] shadow-lg p-8 flex flex-col gap-8 bg-slate-100">
      ...
      <div className="w-full flex justify-between flex-wrap shadow  border-gray-400 px-4 py-4 bg-white">
        ...
        <p className="text-[14px] font-semibold font-sans capitalize">{candidate.workExperience.length>0? candidate.workExperience[0].name:"not available"}</p>
        </div>
        ...
        <div className="flex justify-center items-center px-2 py-1 bg-emerald-600 rounded border border-gray-200"><span className="text-white font-semibold text-[14px] ">{candidate.email? candidate.email : "not available"}</span></div>
      ...
      <div className="flex gap-6 py-0 px-4 mb-4">
        <button onClick={()=>setStep(1)} className={`${Step === 1? "border-b border-cyan-500 font-semibold" :"text-gray-400"}`}>Profile</button>
        <button onClick={()=>setStep(2)} className={`${Step === 2? "border-b border-cyan-500 font-semibold" :"text-gray-400"}`}>Resume</button>
      </div>
      <div onClick={handlePrint}
        disabled={!isReadyToPrint} 
        className="cursor-pointer rounded-full flex items-center justify-center px-4 h-8 gap-2 active:bg-gray-300 select-none border border-gray-200">Download <i className="ri-file-download-line"></i></div>
        </div>
        {Step===1? (
            ...
            <label className="font-bold text-cyan-600 tracking-widest flex  bg-gray-100 py-1 px-2 rounded">Skills</label>
                <div className="flex justify-between px-1 py-1 border-b border-gray-200"><span className="text-gray-600"></span> <span className="text-gray-700 font-semibold tracking-widest ">{candidate.resumeTagline? candidate.resumeTagline:"not available"}</span></div>
            </div>
          ...
          <>
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm font-semibold py-0.5">{candidate.workExperience.length>0? `${candidate.workExperience[0].designation} at ${candidate.workExperience[0].name}`:"Fresher"}</p>  
            </div>
            <div>
            <div className="flex justify-between"><span></span><span className="text-gray-600 text-sm font-semibold">{candidate.email}</span></div>
            ...
            <div className="flex justify-between gap-6 mt-6">
              ...
              <div className={`${candidate.resumeTagline?"border-b w-full border-gray-300 pb-4 flex flex-col gap-1":"hidden"}`}>
                <h2 className="text-[18px] font-semibold">Skills</h2>
                <div className="flex justify-between"><span></span> <span className="text-gray-500">{candidate.resumeTagline}</span></div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 font-semibold">{work.designation}</p>
                </div>
                ))}
                <p className="text-sm text-gray-600 font-semibold"><span className="font-bold text-slate-600">Experience: </span> {candidate.yearsOfExperience? `${candidate.yearsOfExperience} Years`:""}</p>
                <p className="text-sm text-gray-600 font-semibold"><span className="font-bold text-slate-600">CTC: </span> {candidate.currentCtc? `${candidate.currentCtc} `:""}</p>
