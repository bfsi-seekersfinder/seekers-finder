...
router.post("/api/account/login", async (req, res) => {
    try {
        ...
        const user = await recruiterModule.findOne({ email: username });
...
router.get("/api/account/getview", async (req, res)=>{
    try {
        ...
        if(!recruiterId){
        return res.status(401).json({message:"not recruiter found"})
        }
        const user = await recruiterModule.findOne({_id:recruiterId});
        
        const viewCount = user.viewedProfile.length;
        user.totalView = viewCount
        await user.save()
        res.json({message:true, view:{viewCount:user.viewedProfile, totalView:user.totalView}})
    } catch (error) {
        res.status(401).send("view not found")
    }
})
...
router.get('/api/recruiter/gethistory', async (req, res) => {
try {
    const {id} = req.query;
    console.log("id-",id)
...
router.put("/api/recruiter/view-profile", async (req, res) => {
    try {
        ...
        // Check if the profile is already viewed
        const recruiter = await recruiterModule.findById(recruiterId);
        if (!recruiter) {
            return res.status(404).json({ success: false, message: "Recruiter not found" });
        }
        ...
        // Update the recruiter's viewedProfile array
